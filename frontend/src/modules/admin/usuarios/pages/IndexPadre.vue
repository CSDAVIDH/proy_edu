...
import { getPadres, deletePadre } from '@/api/usuarios/padreApi';
...
import CreatePadre from '../components/padres/CreatePadre.vue';
import EditPadre from '../components/padres/EditPadre.vue';
...
const eliminar = (item) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            eliminarPadre.mutate(item);
        }
    });
}

const eliminarPadre = useMutation({
    mutationFn: deletePadre,
    onSuccess: (response) => {
...
<template>
    ...
    <template v-else>
        <CreatePadre :isModal="create" @close="closeCreate" />
        <EditPadre :item="padre" :isModal="edit" @close="closeEdit" />

        <Banner :title="'Lista de Padres de Familia'" />
