...
import { getGrados, updateGrados } from '@/api/grados/gradosApi';
...
const registrarGrado = () => {
    ...
    if (validate.value) {
        Swal.fire({
            ...
            preConfirm: () => {
                loading.value = true;
                actualizarGrado.mutate(form.value);
            },
        });
    } else {
...
const actualizarGrado = useMutation({
    mutationFn: updateGrados,
    onSuccess: (response) => {
...
<template>
    <ModalMd :isModal="isModal" @close="closeModal" :title="`Grado: ${item.nivel}`">
        <form @submit.prevent="registrarGrado" enctype="multipart/form-data">
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        <label class="form-label fw-bold" for="nivel">Nivel de Grado</label>
